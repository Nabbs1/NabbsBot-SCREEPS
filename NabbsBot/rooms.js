		...
		if (room && room.controller && room.controller.my) {
			...
			if (showVisuals == true) {
				if (room.storage) {
					...
					room.visual.text(storPcent + '%', room.storage.pos.x, room.storage.pos.y, {
						font: 'bold 1 HELVETICA',
						align: 'center',
						opacity: 1,
						color: 'white',
						stroke: 'black',
				...
				if (room.memory.colonizeNext != undefined) {
					...
					var scoreActual = topSize - topDistance * 0.5;
				...
				if (spawns.length) {
					...
					if (checkForNext != undefined) {
						let nextClaimRoom =
							checkForNext.room_name + ' | spawn area:' + checkForNext.areaToBuild + ' | source spots:' + checkForNext.sourcePoints;
						room.visual.text('Next Claim: ' + nextClaimRoom, Memory.visLocX, Memory.visLocY + 3, {
							align: 'left',
							opacity: 0.5,
							color: 'white',
							stroke: 'black',
				...
				room.visual.text('GCL: ' + myLevel + ' 🦄 ' + myPcent + '% ', 46, 2, {
					font: 'bold 1.5 HELVETICA',
					align: 'right',
					opacity: 1,
					color: 'white',
					stroke: 'black',
				...
				if (room.memory.scoutRoom != 0) {
					room.visual.text('Scout in room: ' + room.memory.scoutRoom.name, 46, 5, {
						font: 'bold 1.5 HELVETICA',
						align: 'right',
						opacity: 0.5,
						color: 'white',
				...
				room.visual.text(NabbsVersion, Memory.visLocX, Memory.visLocY - 1, {
					font: 'bold 1.5 HELVETICA',
					align: 'left',
					opacity: 1,
					color: 'white',
				...
				room.visual.text(room.name + ' : TotalCreeps: ' + _.size(Game.creeps), Memory.visLocX, Memory.visLocY, {
					font: 'bold 1 HELVETICA',
					align: 'left',
					opacity: 1,
					color: 'white',
				...
				room.visual.text('Lvl: ' + RoomLevel + ' 🦄 ' + roomPcent + '% ', Memory.visLocX, Memory.visLocY + 1, {
					font: 'bold 1 HELVETICA',
					align: 'left',
					opacity: 1,
					color: 'white',
				...
				room.visual.text(room.energyAvailable + ' / ' + room.energyCapacityAvailable, Memory.visLocX, Memory.visLocY + 2, {
					font: 'bold 1 HELVETICA',
					align: 'left',
					opacity: 1,
					color: 'white',
				...
				if (sources.length) {
					_.forEach(sources, function (source) {
						...
						room.visual.text(source.energy + ' : ' + TTR + ' ', source.pos.x, source.pos.y + 1, {
							align: 'left',
							opacity: 0.8,
							stroke: 'black',
							font: '.5',
				...
				let roomHarvesters = room.memory.creepcount.creepCurrent.harvesters;
				let roomDrones = room.memory.creepcount.creepCurrent.drones;
				let roomMiners = room.memory.creepcount.creepCurrent.miners;
				let roomScouts = room.memory.creepcount.creepCurrent.scouts;
				let roomClaimers = room.memory.creepcount.creepCurrent.claimers;
				let roomMamabirds = room.memory.creepcount.creepCurrent.mamabirds;
				let roomUpgraders = room.memory.creepcount.creepCurrent.upgraders;
				room.visual.text('Harvesters: ' + roomHarvesters + ' / ' + harvesterMax, Memory.visLocX, Memory.visLocY + 5, {
				...
				room.visual.text('Drones: ' + roomDrones + ' / ' + droneMax, Memory.visLocX, Memory.visLocY + 6, {
					font: 'bold 0.8 HELVETICA',
					align: 'left',
					opacity: 0.5,
					color: 'white',
				...
				room.visual.text('Miners: ' + roomMiners + ' / ' + minerMax, Memory.visLocX, Memory.visLocY + 7, {
					font: 'bold 0.8 HELVETICA',
					align: 'left',
					opacity: 0.5,
					color: 'white',
				...
				room.visual.text('Mamabird: ' + roomMamabirds + ' / ' + mamabirdMax, Memory.visLocX, Memory.visLocY + 8, {
					font: 'bold 0.8 HELVETICA',
					align: 'left',
					opacity: 0.5,
					color: 'white',
				...
				room.visual.text('Upgraders: ' + roomUpgraders + ' / ' + upgraderMax, Memory.visLocX, Memory.visLocY + 9, {
					font: 'bold 0.8 HELVETICA',
					align: 'left',
					opacity: 0.5,
					color: 'white',
				...
				room.visual.text('Scouts: ' + roomScouts + ' / ' + scoutMax, Memory.visLocX, Memory.visLocY + 10, {
					font: 'bold 0.8 HELVETICA',
					align: 'left',
					opacity: 0.5,
					color: 'white',
				...
				room.visual.text('Claimer: ' + roomClaimers + ' / ' + claimerMax, Memory.visLocX, Memory.visLocY + 11, {
					font: 'bold 0.8 HELVETICA',
					align: 'left',
					opacity: 0.5,
					color: 'white',
...
function sellMarketOrder(room) {
	...
	if (room.terminal && Game.time % 10 == 0) {
		if (room.terminal.store[RESOURCE_ENERGY] >= 2000 && room.terminal.store[RESOURCE_HYDROGEN] >= 2000) {
