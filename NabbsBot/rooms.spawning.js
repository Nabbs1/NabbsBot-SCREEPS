...
function spawning(room) {
	...
	if (!room.memory.creepcount) {
		...
		room.memory.creepcount.creepMax.droneMax = 2;
	...
	//room.memory.creepcount.creepMax.mamabirdMax = 1;
	let dynamicDroneMax = Math.ceil(16 / cLevel);
	let harvesterMax = dynamicDroneMax;
	let droneMax = dynamicDroneMax / 2;
	// let upgraderMax = dynamicDroneMax;
...
//room.energyAvailable + ' / ' + room.energyCapacityAvailable
	...
	if (cLevel > 3 && roomMamabirds < mamabirdMax) {
		...
		let result = spawns[0].spawnCreep(getBody([CARRY, MOVE, CARRY, MOVE, CARRY, MOVE, CARRY, MOVE], room), newName, {
	...
	let thisIsTheRoom;
	let thisRoomSpawn = spawns[0];
	let myLevel = Game.gcl.level;
	let roomClaimsAvailable//mapLib.getGCLClaimsAvailable();
let thisIsNextRoom //= mapLib.getNextClaimableRoom(thisRoomSpawn);
	if (thisIsNextRoom !== undefined) {
		thisIsTheRoom = thisIsNextRoom.room_name;
		//	console.log(room + '[ rooms.spawning ]' + myLevel + ' of ' + roomClaimsAvailable + ' next room: ' + thisIsTheRoom);
	} else {
		//	console.log(room + '[ rooms.spawning ]' + myLevel + ' of ' + roomClaimsAvailable + ' next room: NONE AVAILABLE');
	}
	//	console.log(room+'[ rooms.spawning ]'+myLevel + ' of ' + roomClaimsAvailable + ' next room: ' + thisIsTheRoom);
	...
	if (roomClaimers < 1 && Game.flags.Claim && roomClaimsAvailable > 0 && cLevel > 3 && thisIsNextRoom) {
